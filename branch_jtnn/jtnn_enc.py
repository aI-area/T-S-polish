...
from util.nnutils import create_var, index_select_ND
...
class JTNNEncoder(nn.Module):
    ...
    def forward(self, fnode, fmess, node_graph, mess_graph, scope):
        fnode = create_var(fnode)  # 节点特征
        fmess = create_var(fmess)  # 边特征(边的第一个节点)
        node_graph = create_var(node_graph)  # 接入节点的边
        mess_graph = create_var(mess_graph)  # 接入边的边
        messages = create_var(torch.zeros(mess_graph.size(0), self.hidden_size))  # 边的encoding

        fnode = self.embedding(fnode)
        fmess = index_select_ND(fnode, 0, fmess)
        messages = self.GRU(messages, fmess, mess_graph)  # 图GRU得到边特征

        mess_nei = index_select_ND(messages, 0, node_graph)
        node_vecs = torch.cat([fnode, mess_nei.sum(dim=1)], dim=-1)  # 连接节点特征和边特征
        node_vecs = self.outputNN(node_vecs)  # 得到输出的每个节点特征
    ...
    @staticmethod
    def tensorize_nodes(node_batch, scope):  # node_batch是batch中所有树中所有node, scope记录每棵树nodes在node_batch的下标范围
        messages,mess_dict = [None], {}  # messages记录每个node及其邻居构成的pair , mess_dict 记录每个pair在messages中的下标
        fnode = []  # fnode记录node_batch中node对应于词典的编码
        for x in node_batch:
            fnode.append(x.wid)
        ...
        node_graph = [[] for i in range(len(node_batch))]  # 记录到该id node信息需要汇集到哪些messages边的ids
        mess_graph = [[] for i in range(len(messages))]  # 记录汇集到该message边的messages边的ids
        fmess = [0] * len(messages)  # 记录messages中pair的第一个node对应的id, 这个message是要汇集到哪个节点的
