...
class JTMPN(nn.Module):
    ...
    def forward(self, fatoms, fbonds, agraph, bgraph, scope, tree_message): #tree_message[0] == vec(0)
        ...
        if fbonds.size()[0] == 0:
            fbonds = torch.zeros([1, ATOM_FDIM + BOND_FDIM])
    ...
    def tensorize(cand_batch, mess_dict):
        ...
        # 对应候选的smile， 所有node，当前有多种候选组合的node
        ...
        # TODO: zyj fix: if fatoms is empty, use torch.stack will cause exception
        if len(fatoms) == 0:
            fatoms = torch.tensor(fatoms)
        else:
            fatoms = torch.stack(fatoms, 0)
        if len(fbonds) == 0:
            fbonds = torch.tensor(fbonds)
        else:
